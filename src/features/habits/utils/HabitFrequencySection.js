...
import { ORDINALS, DAYS_OF_WEEK, MONTH_NAMES_SHORT as MONTHS } from '../../../shared/constants.js';
import { capitalize as sharedCapitalize } from '../../../shared/common.js';
import { FREQUENCIES } from '../../../shared/constants.js';
...
function syncDateButtonsWithSet(set) {
  document.querySelectorAll('.date-button').forEach((btn) => {
    const idx = parseInt(btn.dataset.date, 10);
    if (set.has(idx)) btn.classList.add('selected');
    else btn.classList.remove('selected');
  });
}

function syncMonthButtonsWithSet(set) {
  document.querySelectorAll('.month-button').forEach((btn) => {
    const idx = parseInt(btn.dataset.month, 10);
    if (set.has(idx)) btn.classList.add('selected');
    else btn.classList.remove('selected');
  });
}
...
export function handleFrequencyChange() {
  ...
  if (freq === 'monthly') {
    syncDateButtonsWithSet(selectedMonthlyDates);
    updateMonthlyDatesCount();
  }
  if (freq === 'yearly') {
    syncMonthButtonsWithSet(selectedMonths);
    updateMonthsCount();
...
export function updateCombinationsList() {
  ...
  selectedCombinations.forEach((comb) => {
    ...
    text.textContent = `${sharedCapitalize(ord)} ${sharedCapitalize(day)}`;
...
function buildDayButtonsGrid() {
  ...
  labels.forEach((label, idx) => {
    const btn = document.createElement('button');
    btn.className =
      'day-button flex-1 h-12 rounded-lg border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium text-sm hover:border-ios-blue transition-colors';
    btn.dataset.day = idx;
    btn.textContent = label;
    frag.appendChild(btn);
  });
...
function buildCombinationSelectors() {
  ...
  if (ordinalSel && !ordinalSel.dataset.enhanced) {
    ...
    ORDINALS.forEach((ord) => {
      const opt = document.createElement('option');
      opt.value = ord;
      opt.textContent = sharedCapitalize(ord);
  ...
  if (daySel && !daySel.dataset.enhanced) {
    ...
    DAYS_OF_WEEK.forEach((day) => {
      const opt = document.createElement('option');
      opt.value = day;
      opt.textContent = sharedCapitalize(day);
...
function buildFrequencySelect() {
  ...
  FREQUENCIES.forEach((freq) => {
    const opt = document.createElement('option');
    opt.value = freq;
    opt.textContent = sharedCapitalize(freq);
    select.appendChild(opt);
  });
  select.dataset.enhanced = '1';
}

...
function buildColorGrids() {
  ...
  if (addGrid) {
    ...
    COLOR_PALETTE.forEach((color) => {
      ...
      btn.type = 'button';
      btn.className = 'color-option w-8 h-8 rounded-full border-2 border-gray-300 hover:scale-110 transition-transform';
  ...
  if (editGrid) {
    ...
    COLOR_PALETTE.forEach((color) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'edit-color-option w-8 h-8 rounded-full border-2 border-gray-300 hover:scale-110 transition-transform';
      btn.style.backgroundColor = color;
      btn.dataset.color = color;
...
function deleteCategory() {
  ...
  const message = habitsInCategory.length > 0

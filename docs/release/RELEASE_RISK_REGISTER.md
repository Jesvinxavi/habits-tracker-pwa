...
| Production identity | **Partly closed, 1 August 2026 (Jesvin).** Convex is separated: the deployed app now reads the production deployment `hushed-elephant-959`, its schema deploys automatically on publish, and a missing deploy key fails the build instead of being skipped. Clerk is **not** separated — the deployed app still uses a development instance. | Convex half closed. Clerk half **knowingly accepted, not deferred pending work**: production requires a domain the account owner controls, which `jesvinxavi.github.io` is not, and Jesvin decided on 1 August 2026 not to pursue one for now. The accepted cost is the "Development mode" badge and Clerk's development usage limits, which constrain how many users this can carry. Revisit if the app takes on users beyond a small circle. Steps kept ready in `docs/operations/CLERK_PRODUCTION_INSTANCE_BRIEF.md`. |
